...
export default function MeusDadosPage() {
  ...
  function handleEditar() {
    setNomeInput(nomeExibido ?? "");
    setWhatsappInput(whatsappExibido ?? "");
    setErroSalvar(null);
    setEditando(true);
  }
  ...
  async function handleSalvar(event: React.FormEvent) {
    ...
    const { error } = await supabase.auth.updateUser({
      data: { nome: nomeInput.trim(), whatsapp: whatsappInput.trim() },
    });

    setSalvando(false);

    if (error) {
      setErroSalvar("Não foi possível salvar seus dados. Tente novamente.");
      return;
    }
    ...
    <main id="conteudo" className="container-site py-10 sm:py-14">
      <div className="mx-auto max-w-sm">
        <div className="rounded-card border border-border bg-papel p-8 shadow-card">
          ...
          {editando ? (
            <form onSubmit={handleSalvar} className="mt-8 text-center text-[15px]">
              <label className="block">
                <span className="text-marrom-suave">Nome</span>
                <input
                  type="text"
                  required
                  value={nomeInput}
                  onChange={(e) => setNomeInput(e.target.value)}
                  className="mt-1 w-full rounded-lg border border-border bg-areia px-3 py-2 text-center"
                />
              </label>
              <label className="mt-4 block">
                <span className="text-marrom-suave">WhatsApp</span>
                <input
                  type="tel"
                  value={whatsappInput}
              ...
              <div className="mt-4 flex items-center justify-between gap-2 border-t border-border py-3">
              ...
              <div className="mt-4 flex items-center justify-end gap-4">
                <button
                  ...
                  disabled={salvando}
                  className="text-marrom-suave hover:text-vinho"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={salvando}
                  className="inline-flex min-h-11 items-center rounded-pill bg-ambar-escuro px-5 font-semibold text-papel hover:bg-ambar-pressed disabled:opacity-50"
                >
